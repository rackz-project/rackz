...
**5 changes across 2 files. Everything else in `cryptonote_config.h` stays at
upstream values** — the strongest signal that these are deliberate, reasoned
decisions rather than arbitrary tweaks.
...
4. Replace hardfork table — start at v16 (red-zone, one commit with `hardforks:` prefix)
5. Tokenomics: bump `FINAL_SUBSIDY_PER_MINUTE` to `600000000000` (red-zone, `cryptonote_config.h:` prefix)
6. Rename all CMake binary `OUTPUT_NAME` values (green-zone)
7. Change wallet file magic strings and URI scheme (yellow-zone, run wallet tests)
8. Seed node / DNS infrastructure updates (non-red-zone)
9. Version string / release name / display strings (non-red-zone)
10. Record all of the above in `CHANGELOG.md` under `## [Unreleased] → Added/Changed`
